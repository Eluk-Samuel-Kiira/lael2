...
class SendWeeklyReportCommand extends Command
{
    ...
    protected $description = 'Send weekly summary report to admin users at 8pm EAT';
    ...
    public function handle()
    {
        ...
        foreach ($tenants as $tenant) {
            ...
            try {
                if ($locationId) {
                    $results = $this->reportService->sendReport('weekly', $tenant->id, $locationId, $channels);
                    ...
                    if ($locations->isEmpty()) {
                        $results = $this->reportService->sendReport('weekly', $tenant->id, null, $channels);
                        $totalSent += count($results);
                        $this->info("  ✅ Sent tenant-wide weekly report to " . count($results) . " admins");
                    } else {
                        foreach ($locations as $location) {
                            $results = $this->reportService->sendReport('weekly', $tenant->id, $location->id, $channels);
                            $totalSent += count($results);
                            $this->info("  ✅ Sent weekly report for location: {$location->name} to " . count($results) . " admins");
                        }
                    }
                }
            } catch (\Exception $e) {
                ...
                Log::error("Weekly report failed for tenant {$tenant->id}: " . $e->getMessage());
            }
        }

        $this->newLine();
        $this->info("✅ Weekly Report Generation Complete!");
