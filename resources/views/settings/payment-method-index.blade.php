<x-app-layout>
    @section('title', __('payments._payment_methods_index'))
    @section('content')
        ...
        <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-4 gap-md-0">
            ...
            <div class="page-title d-flex flex-column">
                <h1 class="page-heading d-flex text-gray-900 fw-bold fs-2hx fs-md-1 flex-column my-0">
                    {{__('payments._payment_methods_table')}}
                </h1>
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item text-muted">{{__('payments._payment_methods')}}</li>
                </ul>
            </div>
            ...
            <div class="d-flex flex-column flex-sm-row align-items-stretch align-items-sm-center gap-3 w-100 w-md-auto">
                ...
                <div class="w-100 w-sm-250px">
                    <x-liveblade-search 
                        id="paymentMethodSearchInput"
                        componentId="paymentMethodIndexTable"
                        route="{{ route('paymentmethod.index') }}"
                        placeholder="{{__('auth._search')}} {{__('payments._payment_methods')}}"
                    />
                </div>

                <!-- Add Button -->
                @can('create payment method')
                <button type="button" class="btn btn-primary flex-shrink-0" data-bs-toggle="modal" data-bs-target="#kt_modal_add_payment_method">
                    <i class="ki-duotone ki-plus fs-2 me-2 me-sm-3"></i>
                    <span class="d-none d-sm-inline">{{__('payments.new_payment_method')}}</span>
    ...
    <div class="d-flex flex-column flex-column-fluid">
        <div id="kt_app_content" class="app-content flex-column-fluid">
            <div id="kt_app_content_container" class="app-container container-xxl">
                <div id="status"></div>
                <div class="card">
                    @include('settings.payment-method.component')
                </div>
            </div>
        </div>
    </div>
