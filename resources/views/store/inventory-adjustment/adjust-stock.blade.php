            ...
            <div class="modal-body">
                <form id="adjustStockForm{{$item->id}}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="overallQty{{$item->id}}" name="overal_quantity_at_hand" value="{{ $item->variant->overal_quantity_at_hand }}" readonly>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">{{ __('passwords.current_stock_level') }}</label>
                        <input type="text" class="form-control" id="currentStock{{$item->id}}" name="current_quantity" value="{{ $item->quantity_allocated }}" readonly>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">{{ __('passwords.adjustment_amount') }}</label>
                        <input type="number" class="form-control text-center fs-3 fw-bold text-gray-800 quantity-input" id="adjustAmount{{$item->id}}" name="adjust_amount" value="0" data-item-id="{{$item->id}}" data-overall="{{ $item->variant->overal_quantity_at_hand }}" data-current="{{ $item->quantity_allocated }}">
                    </div>

                    <div class="mb-3">
                        <label class="form-label">{{ __('passwords.new_stock_level') }}</label>
                        <input type="text" class="form-control" id="newStock{{$item->id}}" name="new_quantity" value="{{ $item->quantity_allocated }}" readonly>
                    </div>
